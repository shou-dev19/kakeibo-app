...
function generateMonthlySummaryReport(year, month) {
  ...
  sheet
    .getRange(row, 1, 1, 2)
    .setValues([["収支", totalIncome - totalExpense]]);
  row += 2;
  ...
  const chartRange = sheet.getRange(
    `A${row - Object.keys(summary).length}:B${row - 1}`
  );
...
function generateTransactionListReport(year, month) {
  ...
  sheet
    .getRange(2, 1, transactions.length, transactions[0].length)
...
function generateAssetTransitionGraph() {
  // 1. 預金残高の履歴を取得
  const bankTxSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    SHEET_NAMES.DB_TRANSACTIONS
  );
  const bankTransactions =
    bankTxSheet.getLastRow() < 2
      ? []
      : bankTxSheet
          .getRange(2, 1, bankTxSheet.getLastRow() - 1, 8)
          .getValues()
          .filter((tx) => tx[7] !== null && tx[7] !== "");

  // 2. 証券残高の履歴を取得
  const securitiesSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("DB_Securities");
  const securitiesHistory =
    securitiesSheet.getLastRow() < 2
      ? []
      : securitiesSheet
          .getRange(2, 1, securitiesSheet.getLastRow() - 1, 3)
          .getValues();

  if (bankTransactions.length === 0 && securitiesHistory.length === 0) {
    SpreadsheetApp.getUi().alert(
      "残高データがありません。資産推移グラフは生成できません。"
    );
    return;
  }
  ...
  const uniqueDateStrings = [
    ...new Set(
      allDates.map((d) => Utilities.formatDate(d, "JST", "yyyy-MM-dd"))
    ),
  ].sort();
  ...
  for (const dateStr of uniqueDateStrings) {
    ...
    // 合計を計算
    const totalBankBalance = Object.values(lastBankBalances).reduce(
      (sum, b) => sum + b,
      0
    );
    const totalSecuritiesBalance = Object.values(lastSecuritiesBalances).reduce(
      (sum, v) => sum + v,
      0
    );
  ...
  const reportSheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(
    "Report_AssetTransition"
  );
  reportSheet.clear();
...
function generatePortfolioReport() {
  ...
  if (bankTxSheet.getLastRow() >= 2) {
    const bankTransactions = bankTxSheet
      .getRange(2, 1, bankTxSheet.getLastRow() - 1, 8)
      .getValues()
      .filter((tx) => tx[7] !== null && tx[7] !== "");
    ...
    totalBankBalance = Object.values(latestBankBalances).reduce(
      (sum, b) => sum + b,
      0
    );
  }
  ...
  if (securitiesSheet.getLastRow() >= 2) {
    const securitiesHistory = securitiesSheet
      .getRange(2, 1, securitiesSheet.getLastRow() - 1, 3)
      .getValues();
    ...
    totalSecuritiesBalance = Object.values(latestSecuritiesBalances).reduce(
      (sum, v) => sum + v,
      0
    );
  }
  ...
  if (totalBankBalance === 0 && totalSecuritiesBalance === 0) {
    SpreadsheetApp.getUi().alert(
      "残高データがありません。ポートフォリオレポートは生成できません。"
    );
    return;
  }
  ...
  // 3. シートに出力してグラフを作成
  const reportSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Report_Portfolio");
  reportSheet.clear();
