...
- **多彩なレポート機能**:
  - **月次レポート**: 指定した月の収入・支出の合計や、カテゴリ別の支出一覧を円グラフ付きで自動生成します。「投資」など、特定のカテゴリを収支計算から除外する設定も可能です。
  - **年間レポート**: 直近 1 年間の月別収支の推移と、年間でのカテゴリ別支出の平均・合計をテーブルと円グラフで可視化します。こちらも特定のカテゴリを除外して分析できます。
...
2.  以下のシートが自動で作成されます。
    - `取引履歴DB`: 全ての取引履歴が保存されるデータベース
    - `証券残高DB`: 証券口座の評価額を手動で記録するシート
    - `分類・除外設定`: カテゴリ分類ルールや、収支計算から除外するカテゴリを定義するシート
    - `CSVフォーマット設定`: インポートする CSV のフォーマットを定義する設定シート
    - `割り勘キーワード設定`: 割り勘・全額請求のキーワードを定義する設定シート
    - `月次レポート`: 月次レポートの出力先
    - `月次明細一覧`: 月次取引明細の出力先
    - `総資産推移グラフ`: 総資産推移グラフの出力先
    - `割り勘計算レポート`: 割り勘計算結果の出力先
    - `資産ポートフォリオ`: 保有資産ポートフォリオの出力先
    - `年間レポート`: 直近一年間の収支レポートの出力先
...
- **`証券残高DB`シート**: お持ちの証券口座について、「日付」「証券会社名」「その日の評価額」を手動で記録します。定期的に記録することで、より正確な資産推移がグラフに反映されます。
- **`CSVフォーマット設定`シート**: インポートしたい CSV のフォーマットを定義します。`FormatName`には、後述する Google Drive のフォルダ名と一致する名前をつけます。日付、内容、支出額、収入額、残高がそれぞれ何列目にあるかなどを指定します。
- **`分類・除外設定`シート**: 「キーワード」と「カテゴリ」の対応表を作成します。また、「収支から除外するカテゴリ」や「年間レポートから除外するカテゴリ」列に、各レポートの計算に含めたくないカテゴリ名（例: 投資）をリストアップします。
- **`割り勘キーワード設定`シート**: 「割り勘キーワード (50%)」と「全額請求キーワード (100%)」をそれぞれリストアップします。
...
- 「家計簿アプリ」メニューから、以下の機能をいつでも実行できます。
  ...
  - **年間レポートを生成**: 直近 1 年間の月別収支推移や、カテゴリ別支出の分析レポートを作成します。
