...
- **Per-user data (`owner`):** `transactions`, `securities_balances` and `category_rules` carry an `owner` (`'husband' | 'wife'`). Two rules govern it: **read scopes** come from a `?owner=husband|wife|all` query (default `all` = combined); **writes** (import, category rules, re-categorization, securities) always derive the owner from the login and never accept it from the request. Transaction dedupe is `UNIQUE(owner, import_hash)` — the hash formula itself is unchanged so old CSVs still de-duplicate. Asset totals key on `owner + institution/brokerage`, not the name alone. `split_rules`, `csv_formats` and `excluded_categories` are shared by both users; so is the category *name* vocabulary (`getCategories` is deliberately not owner-scoped).
- **Transaction categorization** is keyword + institution rule based (`category_rules`, per-user), with `priority` where **smaller numbers win** (default 100). Bulk re-categorization is scoped to the acting user, offers a dry-run preview, and records `recategorize_runs`/`recategorize_changes` so the latest run can be undone.
- **Splitwise (割り勘)** matches transactions from **both** users against `split_rules` (`match_type` = `keyword`|`institution`, substring match) to assign a rate (%). `rate` means **妻の負担率** and does not depend on who paid; 夫負担額 is derived as `総額 − 妻負担額` so the two always sum to the total. On multiple matches, resolution is by `priority` ascending (**smaller wins**, default 100), then institution-before-keyword, then rate desc, then id — see `shared/splitwise.ts`. Only `type='支出'` and `category !== '振替'` are eligible; unmatched expenses are personal spending and stay out of the split.
...
- `webapp/wrangler.jsonc` holds Cloudflare Workers/D1/Access config. `ALLOWED_EMAILS` (comma-separated) is a Worker Secret controlling access. `OWNER_EMAILS` (`husband:a@x,wife:b@y`) is a second Worker Secret mapping each address to a user; an allow-listed address with no mapping is rejected. Locally, `DEV_OWNER` in `.dev.vars` picks which user you act as while `DEV_BYPASS_ACCESS` is on.
