...
const TABLES = [
  // A full reload replaces every transaction, so any recorded re-categorization
  // run now points at rows that no longer exist. Drop the history with it
  // rather than leaving an "取り消す" button that would silently do nothing.
...
export function buildSql(input: SqlInput): string {
  ...
  parts.push("-- Full reload: wipe every table then re-insert from the spreadsheet export.\n");
  ...
  // The legacy spreadsheet predates per-user data; everything in it is the
  // husband's (see migration 0007).
  const LEGACY_OWNER = "husband";
  ...
  parts.push(
    insertChunks(
      ...
      input.transactions.map((t) => [
        LEGACY_OWNER, t.date, t.description, t.amount, t.type, t.institution, t.category, t.memo, t.balance, t.import_hash,
      ]),
    ),
  );
  ...
  parts.push(
    insertChunks(
      "securities_balances",
      ["owner", "date", "brokerage", "value"],
      input.securities.map((s) => [LEGACY_OWNER, s.date, s.brokerage, s.value]),
    ),
  );
  ...
  parts.push(
    insertChunks(
      "category_rules",
      ["owner", "keyword", "institution", "category", "priority"],
      input.categoryRules.map((r) => [LEGACY_OWNER, r.keyword, r.institution, r.category, r.priority]),
    ),
  );
